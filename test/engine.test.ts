  ...
  plugins: {
    ...
    compileResource({ resource, helper }) {
      ...
      if (resource.data.category) {
        fields.category = resource.data.category as string;
      }
      if (resource.data.author) {
        fields.author = helper.resolveInternalReference(resource.data.author as string);
      }
...
test("builds resources, versions, latest alias, and search indexes", async () => {
  const cwd = await makeFixture({
    ...
    "resources/plugins/test/versions/1.2.0.yaml":
      "type: SoftwareSourceCode\nversion: 1.2.0\ndatePublished: 2024-01-01\n",
    "resources/plugins/test/versions/1.1.0.yaml":
      "type: SoftwareSourceCode\nversion: 1.1.0\ndatePublished: 2023-01-01\n",
  });

  const result = await runBuild({ cwd, write: true, config: makeTestConfig(), mode: "development" }, registry);

  assert.ok(result.documents.some((document) => document.outputPath === "plugins/test/versions/latest/index.json"));
  assert.ok(result.documents.some((document) => document.outputPath === "plugins/search/category/action/index.json"));

  const latest = JSON.parse(await fs.readFile(path.join(cwd, "out/plugins/test/versions/latest/index.json"), "utf8"));
  assert.equal(latest["@id"], "https://example.com/plugins/test/versions/1.2.0");
  ...
  assert.ok(rootIndex.hasPart.some((item: JsonObject) => item["@id"] === "https://example.com/plugins/search"));
  assert.equal(rootIndex["@type"], "DataCatalog");

  const authorsIndex = JSON.parse(await fs.readFile(path.join(cwd, "out/authors/index.json"), "utf8"));
  assert.equal(authorsIndex.itemListElement[0]["@type"], "ListItem");
  assert.equal(authorsIndex.itemListElement[0].item["@type"], "Organization");
  assert.equal(authorsIndex.itemListElement[0].item.name, "Acme Games");

  const pluginsSearchIndex = JSON.parse(
    await fs.readFile(path.join(cwd, "out/plugins/search/category/action/index.json"), "utf8"),
  );
  assert.equal(pluginsSearchIndex.itemListElement[0]["@type"], "ListItem");
  assert.equal(pluginsSearchIndex.itemListElement[0].item["@type"], "SoftwareApplication");
  assert.equal(pluginsSearchIndex.itemListElement[0].item.name, "Test Game");
});
...
test("writes formatted JSON in development mode and minified JSON in production mode", async () => {
  ...
  await runBuild({ cwd, write: true, config: makeTestConfig({ authors: {} }), mode: "development" }, registry);
  const developmentJson = await fs.readFile(path.join(cwd, "out/authors/acme/index.json"), "utf8");
  assert.match(developmentJson, /\n {2}"@context":/);

  await runBuild({ cwd, write: true, config: makeTestConfig({ authors: {} }), mode: "production" }, registry);
  const productionJson = await fs.readFile(path.join(cwd, "out/authors/acme/index.json"), "utf8");
...
test("fails on duplicate YAML keys", async () => {
  ...
  await assert.rejects(() => runBuild({ cwd, write: false, config: makeTestConfig({ authors: {} }) }, registry));
});
...
test("fails with context when a referenced local asset does not exist", async () => {
  ...
  const cwd = await makeFixture({
    ...
    "resources/plugins/test/versions/1.0.0.yaml":
      "type: SoftwareSourceCode\nversion: 1.0.0\nfile:\n  path: /plugins/test/files/missing.zip\n",
  });

  await assert.rejects(
    () => runBuild({ cwd, write: false, config: makeTestConfig({ plugins: {} }), mode: "development" }, assetRegistry),
    (error: unknown) =>
...
test("copies same-origin resource images into the published output path", async () => {
  const cwd = await makeFixture({
    "resources/plugins/test/index.yaml": "type: SoftwareApplication\nname: Test Game\nimage: /plugins/test/index.jpg\n",
    "resources/plugins/test/index.jpg": "jpg payload",
  });

  await runBuild({ cwd, write: true, config: makeTestConfig({ plugins: {} }), mode: "development" }, registry);

  const resource = JSON.parse(await fs.readFile(path.join(cwd, "out/plugins/test/index.json"), "utf8"));
  assert.equal(resource.image, "https://example.com/plugins/test/index.jpg");
  assert.equal(await fs.readFile(path.join(cwd, "out/plugins/test/index.jpg"), "utf8"), "jpg payload");
});
...
test("resolves root-relative resource URLs against the configured root domain", async () => {
  ...
  await runBuild({ cwd, write: true, config: makeTestConfig({ plugins: {} }), mode: "development" }, urlRegistry);

  const resource = JSON.parse(await fs.readFile(path.join(cwd, "out/plugins/test/index.json"), "utf8"));
  assert.equal(resource.url, "https://example.com/plugins/test");
  assert.equal(resource.image, "https://example.com/plugins/test/index.jpg");
});
...
test("fails when a same-origin resource image is missing", async () => {
  ...
  await assert.rejects(
    () => runBuild({ cwd, write: false, config: makeTestConfig({ plugins: {} }), mode: "development" }, registry),
    (error: unknown) =>
...
test("rebuilds latest alias to the next-highest version when the highest version is removed", async () => {
  const cwd = await makeFixture({
    "resources/plugins/test/index.yaml": "type: SoftwareApplication\nname: Test Game\n",
    "resources/plugins/test/versions/2.0.0.yaml":
  ...
  await runBuild({ cwd, write: true, config: makeTestConfig({ plugins: {} }), mode: "development" }, registry);
  let latest = JSON.parse(await fs.readFile(path.join(cwd, "out/plugins/test/versions/latest/index.json"), "utf8"));
  assert.equal(latest.version, "2.0.0");

  await fs.rm(path.join(cwd, "resources/plugins/test/versions/2.0.0.yaml"));
  await runBuild({ cwd, write: true, config: makeTestConfig({ plugins: {} }), mode: "development" }, registry);

  latest = JSON.parse(await fs.readFile(path.join(cwd, "out/plugins/test/versions/latest/index.json"), "utf8"));
...
test("creates empty attribute search indexes when no resources match", async () => {
  ...
  await runBuild(
    { cwd, write: true, config: makeTestConfig({ plugins: { searchAttributes: ["category"] } }), mode: "development" },
    registry,
  );

  const attributeIndex = JSON.parse(
    await fs.readFile(path.join(cwd, "out/plugins/search/category/index.json"), "utf8"),
  );
...
test("fails when a declared resource type is incompatible with its directory resource type", async () => {
  ...
  const cwd = await makeFixture({
    "resources/authors/acme/index.yaml": "type: SoftwareApplication\nname: Wrong Type\n",
  });

  await assert.rejects(
    () =>
      runBuild({ cwd, write: false, config: makeTestConfig({ authors: {} }), mode: "development" }, mismatchRegistry),
...
test("fails with detailed diagnostics for reserved path segments", async () => {
  const cwd = await makeFixture({
    "resources/authors/search/index.yaml": "type: Organization\nname: Reserved\n",
  });

  await assert.rejects(
    () => runBuild({ cwd, write: false, config: makeTestConfig({ authors: {} }), mode: "development" }, registry),
    (error: unknown) =>
      error instanceof Error &&
...
test("generates consistent root, collection, search, and version index documents", async () => {
  const cwd = await makeFixture({
    "resources/authors/acme/index.yaml":
      "type: Organization\nname: Acme Games\ndescription: Publisher\nurl: https://example.com/authors/acme\n",
    "resources/plugins/test/index.yaml": [
      "type: SoftwareApplication",
      "name: Test Game",
      ...
      "category: Action",
      "author: /authors/acme",
      "url: https://example.com/plugins/test",
      "",
    ].join("\n"),
    "resources/plugins/test/versions/1.0.0.yaml": [
      "type: SoftwareSourceCode",
      ...
      "    path: /plugins/test/assets/test-game-1.0.0.zip",
      "    encodingFormat: application/zip",
      "    license: https://spdx.org/licenses/CC0-1.0.html",
      "",
    ].join("\n"),
    "resources/plugins/test/assets/test-game-1.0.0.zip": "zip payload",
  });
  ...
  assert.ok(rootIndex.hasPart.some((h: JsonObject) => h["@id"] === "https://example.com/plugins/search"));
  assert.ok(!rootIndex.about, "root should not have about");

  const collectionIndex = JSON.parse(await fs.readFile(path.join(cwd, "out/plugins/index.json"), "utf8"));
  assert.ok(!collectionIndex.about, "collection should not have about");
  assert.equal(collectionIndex.itemListElement[0].item["@id"], "https://example.com/plugins/test");
  assert.equal(collectionIndex.$schema, "https://example.com/schema/collection");

  const searchManifest = JSON.parse(await fs.readFile(path.join(cwd, "out/plugins/search/index.json"), "utf8"));
  assert.equal(searchManifest.about[0]["@id"], "https://example.com/plugins");
  assert.equal(searchManifest.itemListElement[0].item["@id"], "https://example.com/plugins/search/category");

  const searchValueIndex = JSON.parse(
    await fs.readFile(path.join(cwd, "out/plugins/search/category/action/index.json"), "utf8"),
  );
  assert.equal(searchValueIndex.about[0]["@id"], "https://example.com/plugins");
  ...
  const versionIndex = JSON.parse(await fs.readFile(path.join(cwd, "out/plugins/test/versions/index.json"), "utf8"));
  assert.equal(versionIndex.about[0]["@id"], "https://example.com/plugins/test");
...
test("generates documentation with example requests and example responses", async () => {
  ...
  assert.match(docsHtml, /https:\/\/example\.com\/plugins\/test/);
  assert.match(docsHtml, /https:\/\/example\.com\/plugins\/test\/versions\/1\.0\.0/);
  assert.match(docsHtml, /https:\/\/example\.com\/plugins\/search/);
});
...
test("publishes machine-readable type definitions for generated document shapes", async () => {
  ...
  assert.ok(manifest.definitions.some((entry: { name: string }) => entry.name === "plugins-resource"));
  assert.ok(manifest.definitions.some((entry: { name: string }) => entry.name === "plugins-version"));

  // Check that manifest paths are directory-based (no filenames)
  const pluginsResourceDef = manifest.definitions.find((entry: { name: string }) => entry.name === "plugins-resource");
  assert.equal(pluginsResourceDef.path, "/schema/plugins");
  assert.equal(pluginsResourceDef.url, "https://example.com/schema/plugins");

  const pluginsVersionDef = manifest.definitions.find((entry: { name: string }) => entry.name === "plugins-version");
  assert.equal(pluginsVersionDef.path, "/schema/plugins/versions");
  assert.equal(pluginsVersionDef.url, "https://example.com/schema/plugins/versions");
  ...
  const resourceSchema = JSON.parse(await fs.readFile(path.join(cwd, "out/schema/plugins/index.json"), "utf8"));
  assert.equal(resourceSchema["$id"], "https://example.com/schema/plugins");
  assert.equal(resourceSchema.type, "object");
  assert.ok(resourceSchema.required.includes("@context"));
});

test("resolves author name into internal reference objects", async () => {
  const cwd = await makeFixture({
    "resources/authors/acme/index.yaml": "type: Organization\nname: Acme Games\n",
    "resources/plugins/test/index.yaml":
      "type: SoftwareApplication\nname: Test Game\ncategory: Action\nauthor: /authors/acme\n",
  ...
  await runBuild({ cwd, write: true, config: makeTestConfig({ plugins: {}, authors: {} }) }, registry);

  const game = JSON.parse(await fs.readFile(path.join(cwd, "out/plugins/test/index.json"), "utf8"));
  assert.equal(game.author["@id"], "https://example.com/authors/acme");
  assert.equal(game.author["@type"], "Organization");
  assert.equal(game.author.name, "Acme Games");
});
...
test("includes name in version list items and latest version reference", async () => {
  ...
  await runBuild({ cwd, write: true, config: makeTestConfig({ plugins: {} }) }, registry);
...
test("generates substring name search indexes covering all character positions", async () => {
  ...
  await runBuild(
    {
      ...
      write: true,
      config: makeTestConfig({
        plugins: { searchAttributes: [{ attribute: "name", strategy: "substring" as const }] },
      }),
      mode: "development",
    },
    substringRegistry,
  ...
  const aIndex = JSON.parse(await fs.readFile(path.join(cwd, "out/plugins/search/name/a/index.json"), "utf8"));
  ...
  const alIndex = JSON.parse(await fs.readFile(path.join(cwd, "out/plugins/search/name/al/index.json"), "utf8"));
  ...
  const etIndex = JSON.parse(await fs.readFile(path.join(cwd, "out/plugins/search/name/et/index.json"), "utf8"));
  ...
  const nameManifest = JSON.parse(await fs.readFile(path.join(cwd, "out/plugins/search/name/index.json"), "utf8"));
  assert.ok(
